...
import { ArrowLeft, BookOpen, Trash2, PlusCircle, MinusCircle } from "lucide-react"
import {
    Table,
    TableBody,
    TableCell,
...
export default function CooperativeIncomeExpensePage() {
    ...
    const MonthYearSelector = () => {
        ...
        const months = LAO_MONTHS.map((name, i) => ({ value: i, label: name }));
    ...
    return (
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            ...
            <main className="flex-1 p-4 sm:px-6 sm:py-0 md:gap-8 space-y-4">
                 <div className="grid gap-4 md:grid-cols-2 mb-4 mt-4">
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            {Object.entries(summaryForSelectedPeriod.income).filter(([,v]) => (v as number) !== 0).length > 0 ? (
                                Object.entries(summaryForSelectedPeriod.income).filter(([,v]) => (v as number) !== 0).map(([c, v]) => (
                    ...
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent>
                            {Object.entries(summaryForSelectedPeriod.expense).filter(([,v]) => (v as number) !== 0).length > 0 ? (
                                Object.entries(summaryForSelectedPeriod.expense).filter(([,v]) => (v as number) !== 0).map(([c, v]) => (
