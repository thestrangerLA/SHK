...
interface AddWithdrawalDialogProps {
    ...
    onAddWithdrawal: (withdrawal: Omit<CooperativeWithdrawal, 'id' | 'createdAt' | 'memberName' | 'memberId'>) => Promise<void>;
    memberName: string;
}

export function AddWithdrawalDialog({ open, onOpenChange, onAddWithdrawal, memberName }: AddWithdrawalDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (kip === 0 && thb === 0 && usd === 0 && cny === 0) {
            toast({ title: "ຂໍ້ມູນບໍ່ຄົບຖ້ວນ", description: "ກະລຸນາປ້ອນຈຳນວນເງິນ", variant: "destructive" });
            return;
        }

        try {
            await onAddWithdrawal({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>ຖອນເງິນ: {memberName}</DialogTitle>
                </DialogHeader>
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleSubmit} variant="destructive">ຖອນເງິນ</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
