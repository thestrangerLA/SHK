...
export default function NewLoanPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const isNegativeAmount = Object.values(principalAmount).some(v => (v as number) < 0) || Object.values(repaymentAmount).some(v => (v as number) < 0);
        if(isNegativeAmount) {
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="grid gap-2 md:col-span-1">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 p-2 border rounded-md">
                                        {currencies.map(c => (
                                            <div key={c}>
                                                <Label htmlFor={`amount-${c}`} className="text-xs uppercase">{String(c)}</Label>
                                ...
                                {loanType === 'MURABAHA' ? (
                                    <div className="grid gap-2 md:col-span-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 p-2 border rounded-md">
                                            {currencies.map(c => (
                                                <div key={c}>
                                                    <Label htmlFor={`profit-${c}`} className="text-xs uppercase">{String(c)}</Label>
                                    ...
                                    <div className="grid gap-2 md:col-span-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 p-2 border rounded-md bg-muted/50">
                                            {currencies.map(c => (
                                                <div key={c}>
                                                    <Label htmlFor={`repayment-${c}`} className="text-xs uppercase">{String(c)}</Label>
